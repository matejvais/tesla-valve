...
def initial_guess(Z, bcs, nu):
    ...
    (v, q) = fd.split(w)
    F = (
            2.0*nu * fd.inner(fd.sym(fd.grad(u)), fd.sym(fd.grad(v)))*fd.dx
        - fd.div(v)*p*fd.dx
        - q*fd.div(u)*fd.dx
        )
...
def solve_navier_stokes(ngmsh, orientation="reverse", inlet_velocity_coef=0.1):
    '''
    ...
    Variables:
        ...
        orientation - "reverse" for flow from left to right, "forward" for flow from right to left,
    ...
    z = fd.Function(W)
    if orientation == "reverse":    # fluid flows from left to right (->)
        print("Flow direction: reverse (=>)")
    ...
    elif orientation == "forward": # fluid flows from right to left (<-)
        print("Flow direction: forward (<=)")
    ...
    # Reynolds number
    nu_float = 2.0 # kinematic viscosity
    char_length = 38 # diameter of the pipe
    char_velocity = inlet_velocity_coef*361 # maximum velocity at the inlet
    print(f"Re = {char_velocity*char_length/nu_float}")
    nu_Constant = fd.Constant(nu_float)
    ...
    ds_left = fd.Measure("ds", domain=mesh, subdomain_id=labels_left[0])
    ds_right = fd.Measure("ds", domain=mesh, subdomain_id=labels_right[0])
    ...
    def a(v, u):
        D = fd.sym(fd.grad(v))
        return (fd.inner(fd.grad(v)*v, u) + fd.inner(2*nu_Constant*D, fd.grad(u)))*fd.dx
    ...
    prm = solver.snes  # Solver parameters
    ...
    init = initial_guess(W, bcs, nu=nu_Constant)
    ...
    while t < t_end:
        ...
        # Subtract mean values of the pressure on the left- and right-hand side
        pressure_drop = fd.assemble(p/38*ds_left - p/38*ds_right) 
        pressure_drop = max(pressure_drop, -pressure_drop)

        # Report the pressure drop
        print(f"Pressure drop at timestep t = {t-dt}: {int(pressure_drop)}")
...
if __name__ == "__main__":
    ngmsh = nm.netgen_mesh(lobes=8, max_elem_size=2.5)
    solve_navier_stokes(ngmsh, orientation="reverse", inlet_velocity_coef=0.1)
    solve_navier_stokes(ngmsh, orientation="forward", inlet_velocity_coef=0.1)